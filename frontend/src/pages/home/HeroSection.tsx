...
import { ArrowRight, Globe, Truck, Box, Activity, Zap, ShieldCheck, BarChart3 } from 'lucide-react'
import { motion, useMotionValue, useSpring, useTransform, useScroll } from 'framer-motion'
import gsap from 'gsap'

// --- 3D Floating Element Component ---
const FloatingElement = ({ children, depth = 1, className = "" }: { children: React.ReactNode, depth?: number, className?: string }) => {
...
export default function HeroSection() {
  ...
  return (
    <section ref={containerRef} className="min-h-[110vh] relative flex flex-col items-center justify-center overflow-hidden">
      <AnimatedGrid />
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full text-center pointer-events-none z-0 select-none">
        <div className="text-[18vw] font-black text-gray-900/[0.03] leading-[0.8] tracking-tighter">
          {"LOGISTICS".split('').map((char, i) => (
            <span key={i} className="bg-text-char inline-block">{char}</span>
          ))}
        </div>
      </div>
           ...
           <FloatingElement depth={1.2} className="w-full h-full flex items-center justify-center">
              ...
              >
                ...
                <div className="flex -space-x-2">
                   {[1,2,3,4].map(i => (
                     <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-gray-100 flex items-center justify-center text-[10px] text-gray-400 font-mono">
                       {['US','CN','EU','JP'][i-1]}
                     </div>
                   ))}
                </div>
              ...
              >
                ...
                <div className="h-16 flex items-end gap-1">
                  {[40, 70, 50, 90, 60, 80].map((h, i) => (
                    <div key={i} className="flex-1 bg-[#74B868]" style={{ height: `${h}%`, opacity: i === 5 ? 1 : 0.3, borderRadius: '2px' }} />
                  ))}
                </div>
              </motion.div>
